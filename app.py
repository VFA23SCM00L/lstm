...
@app.route('/api/forecast/issues', methods=['POST'])
def forecast_issues():
    body = request.get_json();
    name_type = 'issues';
    forecast(body,name_type);
...
@app.route('/api/forecast/commits', methods=['POST'])
...
@app.route('/api/forecast/releases', methods=['POST'])
...
def forecast(body,name_type):
    issues = body[name_type]
    type = body["type"]
    repo_name = body["repo"]
    ...
    ALL_ISSUES_DATA_IMAGE_NAME = "all_" + name_type + "_data_" + type + "_"+ repo_name + ".png"
    ...
    axs.legend()
    axs.set_title('All' + name_type + 'Data')
    axs.set_xlabel('Date')
    axs.set_ylabel(name_type)
    # Save the figure in /static/images folder
    ...
    json_response = {
        ...
        "all_" + name_type + "_data_image": ALL_ISSUES_DATA_URL
    }
